...
//        return FALSE;
     ...
     * ----------------------------------------------------------------------------------------------------- +|
     * - OSI 七层模型中  数据在不同层 如信封那样 不断的拆封 当从该层返回时又加封
     *   从外层进入到内层 数据不断精炼去粗 格式转化为内部模型数据 当任务完成后 又去适配外部需要的数据格式
     *   这些验证逻辑 鉴权 格式化|序列化逻辑 都是一些横切 通用功能 最好用类似AOP技术 中间件 动态代理类 钩子|事件来完成
     * ----------------------------------------------------------------------------------------------------- +|
     *
     *
     * @return mixed
