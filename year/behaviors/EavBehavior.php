...
/**
 * TODO 加速查询的话需要使用关系 参考:https://github.com/creocoder/yii2-taggable/blob/master/src/TaggableQueryBehavior.php
 *
 * Class EavBehavior
...
class EavBehavior extends Behavior
{
    ...
    protected function getCacheKey()
    {
        return __CLASS__
        . $this->tableName
        . $this->attributesPrefix
        // . $this->owner->className()->tableName()
    ...
    protected function getModelTableFk()
    {
        ...
        if (empty($this->modelTableFk) || !$this->owner->hasAttribute($this->modelTableFk)) {
            ...
            $this->modelTableFk = $this->owner->primaryKey()[0];
            if (!is_string($this->modelTableFk)) {
                throw new Exception(
                    \Yii::t('yiiext', 'Table "{table}" does not have a primary key defined.',
                        [
    ...
    public function attach($owner)
    {
        ...
        if (!is_string($this->tableName) || empty($this->tableName)) {
            throw new Exception(\Yii::t('yii', 'Property "{class}.{property}" is not defined.',
    ...
    public function saveEavAttributes($attributes)
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ( isset($this->attributes[$attribute]) && !is_null($values = $this->attributes[$attribute])) {
                ...
                $idx = array_search($attributes, $this->changedAttributes);
                if ($idx !== false) {
                    unset($this->changedAttributes[$idx]);
                }
            }
        }
    ...
    public function loadEavAttributes($attributes)
    {
        ...
        foreach ($data as $row) {
            ...
            if (isset($this->attributes[$attribute]) && !is_null($current = $this->attributes[$attribute]) && $current != $value) {
    ...
    public function getEavAttributes($attributes = array())
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ($this->hasSafeAttribute($attribute)) {
                $values[$attribute] = isset($this->attributes[$attribute])? $this->attributes[$attribute] : '' ;
    ...
    {
       /* $_ =  $values = */ $this->getEavAttributes(array($attribute));
    ...
    protected function getLoadEavAttributesCommand($attributes)
    {
        ...
        $query->from($this->tableName)
            ->where([$this->entityField => $this->owner->primaryKey]);
        if (!empty($attributes)) {
            $query->andWhere([
                $this->attributeField => $attributes,
            ]);
        }
        return $query->createCommand($this->owner->getDb());
